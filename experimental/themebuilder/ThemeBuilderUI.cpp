...
void ThemeBuilderUI::setupUiForWidget(const QListWidgetItem* current)
{
  ...
  if ( current ) {
    ...
    es = element_spec_t(); // all fields unset
    es = config->getElementSpec(group, false);

    // common tab
    // inherit cb and combo
    if ( es.inherits.present ) {
    ...
    // frame stuff
    if ( es.frame.hasFrame.present ) {
      frameCb->setChecked(es.frame.hasFrame);
    } else {
      frameCb->setCheckState(Qt::PartiallyChecked);
    }
    ...
    if ( es.frame.element.present ) {
      if ( QString(es.frame.element).isEmpty() ) {
    ...
    if ( es.frame.width.present ) {
      if ( QString("%1").arg(es.frame.width).isEmpty() ) {
    ...
    // interior stuff
    if ( es.interior.hasInterior.present ) {
      interiorCb->setChecked(es.interior.hasInterior);
    } else {
      interiorCb->setCheckState(Qt::PartiallyChecked);
    }
    ...
    if ( es.interior.element.present ) {
      if ( QString(es.interior.element).isEmpty() ) {
    ...
    if ( es.interior.px.present || es.interior.py.present ) {
    ...
    if ( es.label.hmargin.present || es.label.vmargin.present ) {
      labelMarginCb->setCheckState(Qt::Unchecked);
    } else {
      labelMarginCb->setCheckState(Qt::PartiallyChecked);
    }
    ...
    // indicator stuff
    if ( es.indicator.element.present ) {
      if ( QString(es.indicator.element).isEmpty() ) {
...
void ThemeBuilderUI::slot_inheritCbChanged(int state)
{
  ...
  if ( state == Qt::Checked ) {
    ...
    if ( es.inherits.present ) {
      int idx = inheritCombo->findData(QVariant(es.inherits));
      if ( idx == -1 ) {
...
void ThemeBuilderUI::slot_frameIdCbChanged(int state)
{
  ...
  if ( state == Qt::Checked ) {
    if ( es.frame.element.present ) {
      int idx = frameIdCombo->findText(es.frame.element);
      if ( idx == -1 )
        frameIdCombo->addItem(es.frame.element);
      idx = frameIdCombo->findText(es.frame.element);
      frameIdCombo->setCurrentIndex(idx);
      frameIdCombo->setEditText(es.frame.element);
    }
  }
  if ( state == Qt::PartiallyChecked ) {
...
void ThemeBuilderUI::slot_frameWidthCbChanged(int state)
{
  if ( state == Qt::Checked ) {
    ...
    frameWidthSpin->setValue(es.frame.width);
  } else {
...
void ThemeBuilderUI::slot_interiorIdCbChanged(int state)
{
  ...
  if ( state == Qt::Checked ) {
    if ( es.interior.element.present ) {
      int idx = interiorIdCombo->findText(es.interior.element);
      if ( idx == -1 )
        interiorIdCombo->addItem(es.interior.element);
      idx = interiorIdCombo->findText(es.interior.element);
      interiorIdCombo->setCurrentIndex(idx);
      interiorIdCombo->setEditText(es.interior.element);
    }
  }
  if ( state == Qt::PartiallyChecked ) {
...
void ThemeBuilderUI::slot_interiorRepeatCbChanged(int state)
{
  if ( state == Qt::Checked ) {
    ...
    interiorRepeatXSpin->setValue(qMax((int)es.interior.px,interiorRepeatXSpin->minimum()));
    ...
    interiorRepeatYSpin->setValue(qMax((int)es.interior.py,interiorRepeatYSpin->minimum()));
  } else {
...
void ThemeBuilderUI::slot_labelSpacingCbChanged(int state)
{
  if ( state == Qt::Checked ) {
    ...
    labelSpacingSpin->setValue(qMax((int)es.label.tispace,labelSpacingSpin->minimum()));
  } else {
...
void ThemeBuilderUI::slot_labelMarginCbChanged(int state)
{
  if ( state == Qt::Checked ) {
    ...
    labelMarginHSpin->setValue(qMax((int)es.label.hmargin,labelMarginHSpin->minimum()));
    ...
    labelMarginVSpin->setValue(qMax((int)es.label.vmargin,labelMarginVSpin->minimum()));
  } else {
...
void ThemeBuilderUI::slot_indicatorIdCbChanged(int state)
{
  ...
  if ( state == Qt::Checked ) {
    if ( es.indicator.element.present ) {
      int idx = indicatorIdCombo->findText(es.indicator.element);
      if ( idx == -1 )
        indicatorIdCombo->addItem(es.indicator.element);
      idx = indicatorIdCombo->findText(es.indicator.element);
      indicatorIdCombo->setCurrentIndex(idx);
      indicatorIdCombo->setEditText(es.indicator.element);
    }
  }
  if ( state == Qt::PartiallyChecked ) {
